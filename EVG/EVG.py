...
import run_cmd, convert, select_software, merge, bwa, GraphTyper2, \
    ...
    if os.path.isdir(path_dir):
        if force_tmp:  # 如果强制删除，则清空目录后重新创建一个
            ...
            log = "[" + str(datetime.datetime.now()).split(".")[0] + \
                  '] [EVG.makedir] \'{}\' already exists, clear and recreate.\n'.format(path_dir)
            sys.stderr.write(log)
        elif restart_tmp:
            log = "[" + str(datetime.datetime.now()).split(".")[0] + \
                  '] [EVG.makedir] \'{}\' already exists, restart is used, ' \
                  'skipping emptying folders.\n'.format(path_dir)
            sys.stderr.write(log)
        else:  # 如果不强制删除，打印警告并退出代码
            log = "[" + str(datetime.datetime.now()).split(".")[0] + \
                  '] [EVG.makedir] \'{}\' already exists. used --force to overwrite or --restart to restart workflow.\n'.format(path_dir)
            sys.stderr.write(log)
            raise SystemExit(1)
    else:
        os.makedirs(path_dir)
...
def get_samples_path(
    ...
    if len(samples_list) == 0:
        log = "[" + str(datetime.datetime.now()).split(".")[0] + \
              '] [EVG.get_parser] Error: empty file -> {}.\n'.format(samples_file)
        sys.stderr.write(log)
...
def ref_vcf_convert(
    ...
    if log_out:
        return stdout, stderr, log_out, {}
    # 对fasta文件进行评估
    stdout, stderr, log_out, fasta_base = convert.fasta_count(code_dir, reference_file)
    # 如果退出代码有问题，则报错
    if log_out:
        return stdout, stderr, log_out, {}
...
def read_convert(
    ...
    if real_depth == 0:
        log_out = "[" + str(datetime.datetime.now()).split(".")[0] + \
              '] [EVG.run_convert] sequencing depth is 0, please check the input file file\n'
        return stdout, stderr, log_out, {}
    ...
    for sample_name_tmp in sample_name_tmp_list:
        ...
        for software in select_software_list:  # 软件没在结果文件中
            if software not in genotype_outs_map.keys():
                log = "[" + str(datetime.datetime.now()).split(".")[0] + \
                      '] [EVG.genotype] {}: {} results are missing, skipped.\n'.format(sample_name_tmp, software)
