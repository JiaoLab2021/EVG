...
def convert_reference(
    ...
    if restart:
        ...
        if file_size <= 0:
            # 提交任务
            stdout, stderr, log_out = run_cmd.run(cmd, "convert_reference.awk")
    ...
    # 提交任务
    stdout, stderr, log_out = run_cmd.run(cmd, "convert_reference.faidx")
...
def bgzip_vcf(
    ...
    if restart:
        ...
        if getsize(out_vcf_file) <= 28 and getsize(out_vcf_file + ".tbi") <= 72:
            # 提交任务
            stdout, stderr, log_out = run_cmd.run(cmd, "bgzip")
    else:  # 如果没有指定restart，直接运行
        # 提交任务
        stdout, stderr, log_out = run_cmd.run(cmd, "bgzip")
...
def fastq_count(
    ...
    """
    # 代码的路径
    code_path = os.path.join(code_dir, "src", "fastAQ count")
    ...
    # 提交任务
    stdout, stderr, log_out = run_cmd.run(cmd, "fastAQ count")
    ...
    if fastq_base == 0 or read_num == 0 or read_len == 0:
        log = "[" + str(datetime.datetime.now()).split(".")[0] + \
              '] [EVG.fastAQ count] The fastq file is wrong, please check the parameters.\n'
...
def fasta_count(
    ...
    # 提交任务
    stdout, stderr, log_out = run_cmd.run(cmd, "fasta_count")
    ...
    if fasta_base == 0:
        log_out = "[" + str(datetime.datetime.now()).split(".")[0] + \
                  '] [EVG.fasta_count] The fasta file is wrong, please check the parameters.\n'
...
def downsample(
    ...
    # 代码的路径
    code_path = os.path.join(code_dir, "src", "fastAQ sample")
    ...
    if need_ratio >= 1:  # 测序数据小于设定值，跳过，不进行下采样
        log = "[" + str(datetime.datetime.now()).split(".")[0] + \
              '] [EVG.fastAQ sample] Insufficient sequencing data ({:.2f}×/{}×), skip downsampling step.\n'. \
        ...
        if fastq_file2:  # 二代双端测序
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                to_do = []
                future1 = executor.submit(run_cmd.run, cmd1, "fastAQ sample")
                future2 = executor.submit(run_cmd.run, cmd2, "fastAQ sample")
            ...
            # 提交任务
            stdout, stderr, log_out = run_cmd.run(cmd, "fastAQ sample")
...
def vcf_convert(
    ...
    # 代码的路径
    code_path = os.path.join(code_dir, "src", "graphvcf convert")
    ...
    if restart:
        ...
        if getsize(out_name) <= 0 and getsize(out_name + ".gz") <= 0:  # vcf或者bgzip压缩后的文件存在则跳过
            # 提交任务
            stdout, stderr, log_out = run_cmd.run(cmd, "graphvcf convert")
    else:  # 如果没有指定restart，直接运行
        # 提交任务
        stdout, stderr, log_out = run_cmd.run(cmd, "graphvcf convert")
    ...
    if restart:
        ...
        if getsize(out_name) > 0:  # vcf或者bgzip压缩后的文件存在则跳过
            # 提交任务
            stdout, stderr, log_out = run_cmd.run(cmd, "sort")
    else:  # 如果没有指定restart，直接运行
        # 提交任务
        stdout, stderr, log_out = run_cmd.run(cmd, "sort")
    ...
    if mode == "fast":
        ...
        if restart:
            ...
            if file_size <= 0:
                # 提交任务
                stdout, stderr, log_out = run_cmd.run(cmd, "split")
        else:  # 如果没有指定restart，直接运行
            # 提交任务
            stdout, stderr, log_out = run_cmd.run(cmd, "split")
    else:
